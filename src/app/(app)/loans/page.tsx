  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-foreground/60">Зээл</p>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[1.2fr_0.8fr]">
        <Card className="space-y-3">
          ...
          {loans.length === 0 ? (
            <p className="text-sm text-foreground/60">Одоогоор зээл бүртгээгүй байна.</p>
          ) : (
            <div className="space-y-3">
              {loans.map((loan) => (
                <Card key={loan.id} className="border-stroke">
                  <div className="flex items-center justify-between">
                    <div>
        ...
        <Card>
          ...
          <p className="text-sm text-foreground/60">Төлөлт, үлдэгдлээ хянах.</p>
          <div className="mt-4">
            <CreateLoanForm />
          </div>
        </Card>
